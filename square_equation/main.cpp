...
void writeSolution(SquareEquationSolution<Field> solution) {
    if (solution.IsInfinitySolution()) {
        ...
        std::cout << "Корней: " << roots_count << std::endl;
        if (roots_count > 0) {
            std::cout << "Корни: ";
...
int main() {
    double a, b, c;
    std::cout << "Введите коэффциенты a, b, c" << std::endl;
    std::cin >> a >> b >> c;
    std::cout << "a=" << a << ", b=" << b << ", c=" << c << std::endl;

    std::cout << "Решение в вещественных числах" << std::endl;
    auto solution_real = solve_square_equation<double>(a, b, c);
    writeSolution(solution_real);

    std::cout << "Решение в комплексных числах" << std::endl;
